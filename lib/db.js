...
function downloadFile(url, callback) {
  var request = https.get(url, function(stream) {
    callback(null, stream.pipe(zlib.createGunzip()));
  }).on('error', callback);
  // DEBUG
...
var kDelegationEntryPattern = /^([\d\.]+)\t([\d\.]+)\t(\d+)\t(\w+)/mg;
...
function processRegionalDelegationFile(bufs, callback) {
  ...
  for (var match; match = kDelegationEntryPattern.exec(content); ) {
    var networkSize = Prefix.parseIPv4(match[2]) - Prefix.parseIPv4(match[1]);
    var maskLength = Prefix.getMaskLength(~networkSize);
    for (var column = 3; column < 5; column++) {
      if (!delegationsByCountry[match[column]])
        delegationsByCountry[match[column]] = [];

      delegationsByCountry[match[column]].push(new Prefix(match[1], maskLength));
    }
  }
...
function generateDb(options, callback) {
  ...
  var progressBar = false;  // TODO: how to get the length of a gzipped stream?
  if (progressBar) {
