...
var kRegionalDelegationFiles = [{
  ...
  name: 'afrinic',
  url: 'ftp://ftp.afrinic.net/pub/stats/afrinic/delegated-afrinic-extended-latest'
}];
...
function processRegionalDelegationFile(bufs, callback) {
  ...
  for (var match; match = kDelegationEntryPattern.exec(content); ) {
    ...
    if (!delegationsByHash[match[5]])
      delegationsByHash[match[5]] = [];
    var maskLength = Prefix.getMaskLength(~(parseInt(match[4], 10) - 1));
    var prefix = new Prefix(match[3], maskLength);
    delegationsByCountry[match[1]].push(prefix);
    delegationsByHash[match[5]].push(prefix);
  }

  for (var asn in hashByASN) {
    if (!delegationsByHash[hashByASN[asn]])
      continue;
    delegationsByCountry[asn] = delegationsByHash[hashByASN[asn]];
  }
